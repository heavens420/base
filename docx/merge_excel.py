...
path = 'C:\\Users\\420\\Desktop\\kdgc\\new2\\'
# print(os.path.abspath('.'))  # 打印当前目录
xlfs = [path+x for x in os.listdir(path) if os.path.splitext(x)[1] == '.xlsx']  # 罗列目录内所有xlsx文件
...
for n_row in range(1, len(data) + 1):  # 写入数据
    for n_col in range(1, len(data[n_row - 1]) + 1):
        content = str(data[n_row - 1][n_col - 1])
        if content == "None":
            ws.cell(row=n_row, column=n_col, value="")
        else:
            ws.cell(row=n_row, column=n_col, value=content)
