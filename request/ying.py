...
referer = 'https://m.weibo.cn/u/7599147183/home?wvr=5%3Fwvr%3D5&jumpfrom=weibocom'  # referer按照网页自己提供，防止反爬虫
# 设置请求头
headers = {
    'cache-control': 'no-cache',
    # cookie关键数据，
    'cookie': 'UOR=cn.bing.com,weibo.com,cn.bing.com; SINAGLOBAL=4825245536290.708.1644572310201; ULV=1646137596104:4:2:2:58946029018.145386.1646137596096:1646137577524; SUBP=0033WrSXqPxfM725Ws9jqgMF55529P9D9WFqRIIR7pA9whK9.friJdXu5JpX5KMhUgL.FoMf1K.pShMp1he2dJLoIp.LxK.LB-eLB.eLxK.LB-eLB.e_eK.4SK27; ALF=1678794040; SSOLoginState=1647258041; SCF=AuL_S_EOF_5FAPBxnC0gAeIf-xYlNu-P-PfV3K6TUF5YyomyO0QfU-S0Rg2v_x7CIEgNkay5k3UfbTgb1pr8vzI.; SUB=_2A25PK1nqDeRhGeFL4lsQ9CnNwz-IHXVsQcwirDV8PUNbmtANLWHFkW9NfWwn1ytfsoM-Cm5hgkcJDezI7MjO0mQv',
    'referer': referer,
    'transparent': '00-5c81397f0ccf06af48cb59bfd2a68ead-033bf99f9cfd7101-00',
...
def save_data(seq, category, pic_file_midr, result_file_path, detail_text, ten_comments, pic_url, key_word,
              person):  # 按照数据格式保存
    ...
    for page in range(1, 5000):  # 瀑布流下拉式，加载，max_id从0开始
        ...
        for item in items:
            if (item.get('comments_count') >= 20):  # 判断该微博的评论数量是否能达到15条，这里有个问题：评论总数是15条，而不是一级评论是15条以上。
                ...
                isLongText = item.get('isLongText')
                ...
                if ten_comments is not None:
                    if len(pic_url) > 0:
                        if detail_text is not None:
                            ...
                            save_data(seq, category, pic_file_midr, result_file_path, detail_text, ten_comments,
                                      pic_url, key_word, person)
